...
import com.example.activiti.dto.NodeAssignee;
import com.example.activiti.dto.ProcessNode;
import com.example.activiti.service.IProcessEngine;
...
//    private ProcessFormService processFormService;
    ...
    public String start(String deploymentId, String businessId, String startUser) {
        ...
        if (StringUtils.isBlank(deploymentId)) {
            throw new RuntimeException("流程发布ID不允许为空");
        }
        if (StringUtils.isBlank(startUser)) {
            // 获取当前登录用户
            startUser = "test123";
        }
...
//        flowData.setMainFormKey(processForm.getFormCode());
        ...
        taskVariables.put(ProcessUtils.FLOW_DATA_KEY, flowData);
        ...
        if (processDefinition.isSuspended()) {
            throw new RuntimeException("该流程已被挂起，无法使用，请激活后使用");
        }
        ...
        taskVariables.put(ProcessUtils.FLOW_DATA_KEY, flowData);
        // 储存流程表单流转数据
        taskVariables.put(ProcessUtils.FLOW_VARIABLES_KEY, new HashMap<>());
    ...
    public void submitTask(Map<String, Object> flowVariables) {
        ...
        final String currentUser = "test2";
        final FlowData flowData = new FlowData();
        BeanUtils.copyProperties(flowVariables, flowData);

        // 下一节点审批人
        final String nextUser = flowData.getNextUser();
        final String comment = StringUtils.isBlank(flowData.getComment()) ? ProcessUtils.DEFAULT_AGREE_COMMENT : flowData.getComment();
        ...
        final Task currentTask = ProcessUtils.getCurrentTask(taskId);
        final String currentNodeId = currentTask.getTaskDefinitionKey();

        if (StringUtils.isBlank(processInstanceId)) {
            throw new RuntimeException("流程实例ID不允许为空");
        }
        if (StringUtils.isBlank(nextUser) && nextNodeNum != 0) {
            throw new RuntimeException("下一环节审批人不允许为空");
        }
        ...
        variables.put(ProcessUtils.FLOW_VARIABLES_KEY, flowVariables);
...
//            taskService.get
            ...
            ProcessUtils.nodeJumpTo(taskId, flowData.getTargetNodeId(), currentUser, variables, comment);
        } else {
            throw new RuntimeException("提交任务异常");
        }
    ...
    public void withdrawApproval(String oldTaskId) {
        final String currentUser = "cehui";
        ...
        if (ProcessUtils.isFinished(processInstanceId)) {
            throw new RuntimeException("流程已结束，不能撤回");
        }
        ...
        if (tasks.size() == 1) {
            ...
            if (nextNodes.size() > 1) {
                throw new RuntimeException("撤回失败，任务已被提交");
            }
...
//                activityMapper.deleteHisActivityInstanceByTaskId(currentTaskId);
            ...
            throw new RuntimeException("目前暂不支持多任务撤回");
        } else {
            throw new RuntimeException("撤回失败，任务已被提交或不存在");
        }
    ...
    public Map<String, Object> getCurrentUserTaskVariables(String taskId) {
        ...
        Task currentTask = ProcessUtils.getCurrentUserTask(taskId);
        if (currentTask == null) {
            // TODO: 流程完结或当前用户没有权限
            return null;
        }
        ...
        // 获取当前节点表单数据
        final Map<String, Object> flowVariables = ProcessUtils.getFlowVariables(taskId);
        final FlowData flowData = ProcessUtils.getFlowData(taskId);
...
//        }
        ...
        hashMap.put(ProcessUtils.FLOW_DATA_KEY, flowData);
//        hashMap.put(ProcessKit.FORM_CONFIG_KEY, formConfig);

        // 设置下一步路由出口用户任务节点集
        List<UserTask> nextUserTask = ProcessUtils.getNextNode(processDefinitionId, currentNodeId, flowVariables);
        List<ProcessNode> processNodes = ProcessUtils.convertTo(nextUserTask);
        flowData.setNextNodes(processNodes);
        flowData.setNextNodeNum(processNodes.size());

        // 设置当前环节可驳回的所有任务节点集
        List<ProcessNode> canBackNodes = ProcessUtils.getCanBackNodes(currentNodeId, processDefinitionId);
    ...
    public Map<String, Object> getHisUserTaskVariables(String processInstanceId) {
        ...
        // 获取当前节点表单数据
        final Map<String, Object> flowVariables = ProcessUtils.getHisFlowVariables(processInstanceId);
...
//        hashMap.put(ProcessKit.FORM_CONFIG_KEY, formConfig);
    ...
    public void backToFirstNode(String taskId) {
        ...
        ProcessUtils.nodeJumpTo(taskId, flowData.getFirstNodeId(), currentUser, variables, "回退首环节");
    }
    ...
    public void backToPreNode(String taskId) {
        ...
        Map<String, Object> variables = taskService.getVariables(taskId);
        FlowData flowData = ProcessUtils.getFlowData(variables);
        Task currentTask = ProcessUtils.getCurrentTask(taskId);
        // 获取上一环节
        ProcessNode preOneIncomeNode = ProcessUtils.getPreOneIncomeNode(currentTask.getTaskDefinitionKey(), flowData.getProcessDefinitionId());
        if (preOneIncomeNode == null) {
            throw new RuntimeException("驳回失败，preOneIncomeNode空指针异常");
        }
        ...
        if (preNodeId.equals(flowData.getFirstNodeId())) {
            ...
            // 获取目标节点审批人
            int taskType = ProcessUtils.getTaskType(preNodeId, flowData.getProcessDefinitionId());
        ...
        ProcessUtils.nodeJumpTo(taskId, preNodeId, currentUser, variables, "驳回上环节");
    }
    ...
    public void back2Node(String taskId, String targetNodeId) {
        ...
        ProcessUtils.nodeJumpTo(taskId, targetNodeId, currentUser, variables, "驳回【" + flowElement.getName() + "】环节");
    }
}
